...
const FRANNO_FEEDBLOB  = 'fireriver/feedblob';
...
exports.main = function (options, callbacks) {
    ...
    if ('undefined' == typeof(simpleStorage.storage.feeds_ignored)) {
        // TODO: Should this be an array? So I can shift / pop old entries when
        // quota runs out?
...
function handleFeedsFoundForPage (event) {
    ...
    // Have we ignored this host? If so, skip.
    if ('undefined' != typeof(simpleStorage.storage.hosts_ignored[site_url.hostPort])) {
        return;
    }

    // Have we ignored this feed? If so, skip.
    if ('undefined' != typeof(simpleStorage.storage.feeds_ignored[feed.url])) {
        return;
    }
    ...
    appendNotificationBox({
        ...
        buttons: [
            { label: "Never check this site for feeds", 
                accessKey: "n", popup: null, 
                callback: function (bar, button) { ignoreThisHost(event); }
            },
            { label: "Ignore this page", 
                accessKey: "i", popup: null, 
                callback: function (bar, button) { ignoreFeeds(event); }
            },
            { label: "Subscribe to this page", 
                accessKey: "s", popup: null, 
                callback: function (bar, button) { subscribeToPage(event); }
            },
        ]
    });
}
...
function ignoreFeeds (event) {
    ...
    for (var i=0; i<event.items.length; i++) {
        var feed = event.items[i];
        simpleStorage.storage.feeds_ignored[feed.url] = 1;
    }
    // TODO: Preference to give notification? Silent for now.
}
...
function postFeedsUpdate (worker) {
    ...
    for (var i=0; i<root.childCount; i++) {
        ...
        if (!annotationService.itemHasAnnotation(feed_node_id, FRANNO_FEEDBLOB)) {
            console.debug("FEED " + feed_node_id + " " + feed_node.title + " HAS NO BLOB");
            continue;
        }

        var feed_in_data = annotationService.getItemAnnotation(feed_node_id, FRANNO_FEEDBLOB);
        ...
        for (var j=0; j<feed_node.childCount; j++) {
            ...
            if (entry_in) {
                ...
                entry_out.published = entry_in.published;
            }
    ...
    feedProc.listener = {
        ...
        handleResult: function resultListener_handleResult(aResult) {
            ...
            if (annotationService.itemHasAnnotation(folder_id, FRANNO_AUTOADDED)) {
                ...
                // Only correct the title/link once.
                annotationService.removeItemAnnotation(folder_id, FRANNO_AUTOADDED);
            }
            ...
            annotationService.setItemAnnotation(folder_id, FRANNO_FEEDBLOB,
