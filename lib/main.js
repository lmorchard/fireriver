...
exports.main = function (options, callbacks) {
    ...
    widget.Widget({
        label: 'Fireriver',
        width: 36,
        content: '<img src="'+data.url('img/blue-rss.png')+'" width="20" height="20" />&nbsp;',
        onClick: function () {
            openIndex();
        }
    });
...
/** 
 * If open, activate the index tab. If not, open as an app tab in the background. 
 *
 * This is an attempt to keep only one instance of the page open at a time, though
 * multiple instances should be okay.
 */
function openIndex (background) {
    for each (var tab in tabs) {
        if (tab.url == index_url) { return tab.activate(); }
    }
    tabs.open({ url: index_url, isPinned: true, inBackground: !!background });
}
